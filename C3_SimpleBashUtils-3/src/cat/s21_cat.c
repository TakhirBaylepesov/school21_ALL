...
void handler_string(flags f, const char *const str, const size_t len,
  ...
  if (f.b)
    handler_b(str, len, cnt);
  else if (f.s)
    handler_s(str, len, prev);
  else if (f.n)
    handler_n(str, len, cnt);
  else if (f.e)
    handler_e(str, len);
  else if (f.E)
    handler_E(str, len);
...
void handler_t(const char *const str, const size_t len) {
...
void handler_v(const char *const str, const size_t len) {
  for (size_t i = 0; i < len; i++) {
    printf("%s", symb[(int)str[i]]);
  }
}

void handler_e(const char *const str, const size_t len) {
  for (size_t i = 0; i < len; i++) {
    if (str[i] == '\n')
      printf("$\n");
    else if (str[i] == '\t')
...
void handler_b(const char *const str, const size_t len, int *const cnt) {
  char num[SIZE] = {'\0'};
  num_to_str(*cnt, num);
  if (*str != '\n') {
...
void handler_n(const char *const str, const size_t len, int *const cnt) {
  char num[SIZE] = {'\0'};
  num_to_str(*cnt, num);
  int sp = 6 - log10(*cnt) - 0.0005;
  while (sp--) putchar(' ');
...
void handler_s(const char *const str, const size_t len, int *const prev) {
  if (*str == '\n') {
    (*prev)++;
